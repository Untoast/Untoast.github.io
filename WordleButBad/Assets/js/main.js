...
const JsonObjectNames = ["give_up_data", "custom_theming", "page_info"]
...
async function Main() {
    ...
    // getting give up notes
    var JsonNoteData;
    async function awaitData() {
        await fetch(pageInfoJsonPath)
            .then(response => response.json())
            .then(data => {
                JsonNoteData = data[JsonObjectNames[0]];
            });
    }
    await awaitData();
    console.log(JsonNoteData)
    const GiveUpNotes = JsonNoteData["give_up_text"];
    const GibsonNotes = JsonNoteData["gibson_text"];

    // give up note
    const giveUpNoteType = GiveUpNotes["type"];
    const giveUpNote = GiveUpNotes[giveUpNoteType];
    // Gibson note
    const gibsonNoteType = GibsonNotes["type"];
    const gibsonNote = GibsonNotes[gibsonNoteType];
    ...
    async function ApplyRules(Word) {
        if (Guessed) { return; }
        if (Waiting) { Noti("Wait for previous guess to finish...", "#d72929"); return; }
        CurrentIndex++;

        Word = Word.toLowerCase();
        // Test if word had already been submitted
        if (SubmittedWords.includes(Word)) {
            Noti("Word has already been submitted!", "#d72929");
            return;
        }
        if (Word.length != 5) {
            Noti("Word length must be 5 characters!", "#d72929");
            return;
        }
        if (!words.includes(Word)) {
            Noti("Word is not in list!", "#d72929");
            return;
        }
        Waiting = true
        Guesses++
        ...
        for (let i = 0; i < Word.length; i++) {
            ...
            if (CorrectPosition.at(i)) {
                ...
                if (!AlteredKeyboardItems[letterInSubmitted] || AlteredKeyboardItems[letterInSubmitted] == "WrongPos") {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Correct, keyboard: ColorMapping.keyboard.Correct })
                }
                else {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Correct })
                }
            ...
            //Wrong Pos
                ...
                if (Hitsforinput[letterInSubmitted] > amountofeachchar[letterInSubmitted]) { ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Wrong}); continue }
                //To guess = pouch, submitted = mecca, only second c will be green

                if (amountincorrectpos[letterInSubmitted] >= amountofeachchar[letterInSubmitted]) { ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Wrong}); continue }

                if (!AlteredKeyboardItems[letterInSubmitted] || AlteredKeyboardItems[letterInSubmitted] == "WrongPos" && !Found[letterInSubmitted]) {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.WrongPos, keyboard: ColorMapping.keyboard.WrongPos })
                }
                else {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.WrongPos })
                }
                console.log(GetPosInWord(wordToGuess, letterInSubmitted))
                ...
                if (!AlteredKeyboardItems[letterInSubmitted] && !Found[letterInSubmitted]) {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Wrong, keyboard: ColorMapping.keyboard.Wrong })
                }
                else {
                    ColorsForCurrentWord.splice(i, 0, { color: "#ffffff", background: ColorMapping.tiles.Wrong })
                }
        ...
        for (let i = 0; i < Word.length; i++) {
            ...
            if (ColorsForCurrentWord[i].background == ColorMapping.tiles.Correct) { ScoreType = "Correct" }
            else if (ColorsForCurrentWord[i].background == ColorMapping.tiles.WrongPos) { ScoreType = "Semi" }
            ...
            document.getElementById(Word[i]).style.textShadow = "0px 0px 15px rgb(0, 0, 0)"
        ...
        if (Word === wordToGuess) {
            ...
            TransitionNewGame()
            Noti("You successfully guessed " + wordToGuess + " in " + Guesses + " attempts! Press new to play again.", "#75ff75", true)
            Guessed = true
    ...
    Stop.addEventListener("click", (event) => {
        ...
        TransitionNewGame()
        setTimeout(() => {
        Noti(giveUpNote + Guesses + " guesses, the word was \'" + wordToGuess + "\'!", "#d72929", true)
        }, 1000);
    });
    Gibson.addEventListener("click", (event) => {
        ...
        TransitionNewGame()
        setTimeout(() => {
        Noti(gibsonNote + Guesses + " guesses, the word was \'" + wordToGuess + "\'!", "#d72929", true)
        }, 1000);
    });
...
async function Theming(){
    ...
    async function awaitData() {
        await fetch(pageInfoJsonPath)
            .then(response => response.json())
            .then(data => {
                JsonThemeData = data[JsonObjectNames[1]];
            });
    }
    await awaitData();
    console.log(JsonThemeData)
    const customThemingToggleable = JsonThemeData["enabled"];
    const customThemingDefaultToggled = JsonThemeData["default_toggled"];
    const themeName = JsonThemeData["theme_name"];

    // CUSTOM THEMING DEFAULTS
    if (customThemingDefaultToggled) {
        // ----- CUSTOM -----
        oldAudio = true
        oldBG = true
        DefaultAudios = "/CustomAudios"
    }
    else {
        // ----- DEFAULTS -----
        oldAudio = true
    ...
    try {
        if (getSettings()["bg"]["value"] != customThemingDefaultToggled && getSettings()["audio"]["value"] != customThemingDefaultToggled) {
            saveSetting("bg", customThemingDefaultToggled)
            saveSetting("audio", customThemingDefaultToggled)
            ToggleBG(customThemingDefaultToggled)
            ToggleAudio(customThemingDefaultToggled)
        }
    }
    catch {
        saveSetting("bg", customThemingDefaultToggled)
...
// assure bg is hidden on load
window.addEventListener("DOMContentLoaded", () => {
    setTimeout(() => {
        document.getElementById("bg").style.transition = ".5s"
    }, 100);
});
